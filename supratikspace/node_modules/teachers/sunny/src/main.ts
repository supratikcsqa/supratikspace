...
function renderEditOverlay(sheet: SheetRecord): string {
  ...
  return `
    ...
    (color) => `
            ...
            <div class="tool-rail-group">
              <span class="section-kicker">Zoom</span>
              <div class="hero-chip-row" style="margin-bottom: 8px;">
                <button class="subtle-button" data-action="set-zoom" data-zoom-value="${Math.max(0.5, state.zoom - 0.25)}">-</button>
                <span class="info-chip" style="cursor:pointer;" data-action="set-zoom" data-zoom-value="1">${Math.round(state.zoom * 100)}%</span>
                <button class="subtle-button" data-action="set-zoom" data-zoom-value="${Math.min(3.0, state.zoom + 0.25)}">+</button>
              </div>
            </div>
            ...
            <div class="tool-rail-group">
              ...
              <span class="edit-footnote">Scroll stays locked unless you zoom in, so Pencil movement feels direct.</span>
            </div>
          </aside>

          <div class="edit-stage-shell edit-stage-shell-focus" style="overflow: auto;">
            <div
              class="edit-stage-frame"
              data-edit-stage-frame
              style="width:${Math.round(stageSize.width * state.zoom)}px; height:${Math.round(stageSize.height * state.zoom)}px;"
            >
